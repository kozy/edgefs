...
func globalList(bpath string) error {
	ep := edgex.GlobalGetPrepare(bpath)
	if ep != nil {
...
var (
	globalListCmd = &cobra.Command{
		Use:   "globallist  <cluster>/<tenant>/<bucket>/<kvobject>",
		Short: "globallist key/value pairs",
		Long:  "globallist key/value pairs from kvobject",
		Args:  validate.KeyValue,
		Run: func(cmd *cobra.Command, args []string) {
			err := globalList(args[0])
			if err != nil {
				fmt.Println(err)
				os.Exit(1)
			}
...
func init() {
	KeyValueCmd.AddCommand(globalListCmd)
}
