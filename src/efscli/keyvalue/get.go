...
func getByKey(opath string, key string) error {
	if key == "" {
...
var (
	getCmd = &cobra.Command{
		Use:   "get  <cluster>/<tenant>/<bucket>/<object> <key>",
		Short: "get value by key",
		Long:  "get value from kvobject by key",
		Args:  validate.Object,
		Run: func(cmd *cobra.Command, args []string) {
			err := getByKey(args[0], args[1])
			if err != nil {
				fmt.Println(err)
				os.Exit(1)
			}
