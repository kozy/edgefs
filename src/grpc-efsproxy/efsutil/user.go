...
func ListUser(cluster string, tenant string, count int, name string) ([]*User, error) {
	...
	ret := C.ccow_tenant_init(c_conf, c_cluster, C.strlen(c_cluster)+1,
		c_tenant, C.strlen(c_tenant)+1, &tc)
	if ret != 0 {
		return res, fmt.Errorf("ccow_tenant_init err=%d", ret)
	}
	defer C.ccow_tenant_term(tc)
	...
	if ret != 0 {
		return res, fmt.Errorf("Get user list error=%d", ret)
	}
	...
	for {
		...
		if ver != 2 {
			return res, fmt.Errorf("Unpack user version error=%d", ver)
		}
		...
		ret = C.int(C.msgpack_unpack_str(u, c_buf, buf_size-1))
		if ret != 0 {
			return res, fmt.Errorf("Unpack user buffer error=%d", ret)
		}
